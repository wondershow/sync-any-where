...
int tcpsend_file_to_remote_peer4demo(char *filename,int port)
{
    int sd,cd; 
...
void *tcp_sync_client()
{
	...
	printf("tcp_sync_client:nothing to send, I am sleeping:\n");
	  ...
	  getCurrentUnsyncedFile4DemoTCP(file_name,&file_len,dest_ip,&tcp_port);
	  
	  
	  
	  
	  //The first step is to get the TCP port of remote peer//removed for demo
	  ...
	  //tcpsend_file_to_remote_peer(file_name,dest_ip,tcp_port);/** To be removed for demo, **/
	  tcpsend_file_to_remote_peer4demo(file_name,tcp_port);
	  sleep(5);
      }
    }
}